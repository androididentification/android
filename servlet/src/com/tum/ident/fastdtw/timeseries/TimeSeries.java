      ...
      try
      {
         ...
         if (isLabeled)
         {
            ...
            if (labels.size() == 0)
               ...
               labels.add(0, "Time");
            else if (isFirstColTime && !labels.get(0).equalsIgnoreCase("Time"))
...
//               timeValueMap.put(timeReadings.get(timeReadings.size()-1), readings);
 ...
 //     timeValueMap.clear();
   ...
   public void addLast(double time, TimeSeriesPoint values)
   {
      ...
      if ( (this.size()>0) && (time<=timeReadings.get(timeReadings.size()-1).doubleValue()) )
...
private static boolean determineIsFirstColTime(String filePath)
   {
      ...
      try
      {
         ...
         final Vector<Double> possibleTimeValues = new Vector<Double>(NUM_OF_VALUES_TO_CMP);  // 'stores numOfValuesToCompare' values
         ...
         for (int x=1; x<possibleTimeValues.size(); x++)
         {
            if (Math.abs( possibleTimeValues.get(x).doubleValue() -
                          possibleTimeValues.get(x-1).doubleValue() - expectedDiff)
